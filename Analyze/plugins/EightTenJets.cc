...
EightTenJets::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  // using namespace edm;
   ...
   //   printf("jet  with pt %5.1f (raw pt %5.1f), eta %+4.2f, btag CSV %.3f, CISV %.3f, pileup mva disc %+.2f\n",
	...
	SphericityAplanarity(_sphericity, _aplanarity, _px, _py, _pz);
    ...
    if ( _isMC )
    {
		...
		SphericityAplanarity(_partonSphericity, _partonAplanarity, _partonPx, _partonPy, _partonPz);
		...
		SphericityAplanarity(_genSphericity, _genAplanarity, _genPx, _genPy, _genPz);
